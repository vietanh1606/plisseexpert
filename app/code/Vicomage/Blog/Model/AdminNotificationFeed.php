...
class AdminNotificationFeed extends \Magento\AdminNotification\Model\Feed
{
    ...
    public function getFeedUrl()
    {
        ...
        $modulesParams = array();
        foreach($this->getVicomageModules() as $key => $module) {
            $key = str_replace('Vicomage_', '', $key);
    ...
    /**
     * Get Vicomage Modules Info
     *
     * @return $this
     */
    protected function getVicomageModules()
    {
        $modules = array();
        foreach($this->_moduleList->getAll() as $moduleName => $module) {
            if ( strpos($moduleName, 'Vicomage_') !== false && $this->_moduleManager->isEnabled($moduleName) ) {
    ...
    {
        return $this->_cacheManager->load('Vicomage_admin_notifications_lastcheck');
    }
    ...
    {
        $this->_cacheManager->save(time(), 'Vicomage_admin_notifications_lastcheck');
        return $this;
    }
