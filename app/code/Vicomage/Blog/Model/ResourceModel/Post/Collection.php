...
namespace Vicomage\Blog\Model\ResourceModel\Post;
...
class Collection extends \Magento\Framework\Model\ResourceModel\Db\Collection\AbstractCollection
{
    ...
    {
        parent::_construct();
        $this->_init('Vicomage\Blog\Model\Post', 'Vicomage\Blog\Model\ResourceModel\Post');
     ...
     * @param array|int|\Vicomage\Blog\Model\Category  $category
     * @return $this
     */
    public function addCategoryFilter($category)
    {
        if (!$this->getFlag('category_filter_added')) {
            if ($category instanceof \Vicomage\Blog\Model\Category) {
     ...
     * @param array|int|\Vicomage\Blog\Model\Author  $author
     * @return $this
     */
    public function addAuthorFilter($author)
    {
        if (!$this->getFlag('author_filter_added')) {
            if ($author instanceof \Vicomage\Blog\Model\Author) {
                $author = [$author->getId()];
            }
    ...
    protected function _afterLoad()
    {
        ...
        if (count($items)) {
            ...
            $select = $connection->select()->from(['cps' => $this->getTable('vicomage_blog_post_store')])
            ...
            $select = $connection->select()->from(['cps' => $this->getTable('vicomage_blog_post_category')])
                ->where('cps.post_id IN (?)', $items);
    ...
    protected function _renderFiltersBefore()
    {
        foreach(array('store', 'category') as $key) {
            if ($this->getFilter($key)) {
                $this->getSelect()->join(
                    [$key.'_table' => $this->getTable('vicomage_blog_post_'.$key)],
