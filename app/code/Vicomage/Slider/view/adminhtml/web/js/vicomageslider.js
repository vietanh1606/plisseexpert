...
define([
    ...
    $.widget('mage.vicomageSlider', {
        ...
        findElement: function (data) {
            return this.element.find(this.options.imageSelector).filter(function () {
                if($(this).data('imageData').file) {
                    return $(this).data('imageData').file === data.file;
                }else{
                    return $(this).data('imageData').image === data.image;
                }
            }).first();
        },
        ...
        _addItem: function (event, imageData) {
            ...
            imageData = $.extend({
                ...
                'media_type': imageData.media_type ? imageData.media_type : 'image',
            }, imageData);
    ...
    $.widget('mage.vicomageSlider', $.mage.vicomageSlider, {
        options: {
        ...
        _create: function () {
            ...
            this._super();
            this.videoDialog = this.element.find('#vicomage-new-video');
            this.videoDialog.mage('vicomageNewVideoDialog', this.videoDialog.data('modalInfo'));
        ...
        _initDialog: function () {
            ...
            $dialog.on('change', '[data-role="image-label"]', function (e) {
                ...
                this.element.find('input[type="hidden"][name="' + targetName + '"]').val(label);

                imageData.label = label;
                imageData.label_default = label;
            ...
            $dialog.on('change', '[data-role="image-urlcontent"]', function (e) {
                var target = $(e.target),
                    targetName = target.attr('name'),
                    urlcontent = target.val(),
                    imageData = $dialog.data('imageData');
                imageData.urlcontent = urlcontent;
                this.element.find('input[type="hidden"][name="' + targetName + '"]').val(urlcontent);
            }.bind(this));
            ...
            $(event.target)
                ...
                .each($.proxy(function (index, checkbox) {
                    var $checkbox = $(checkbox),
                        ...
                        selectedClass = 'selected',
                        isChecked = this.options.types[$checkbox.val()].value == imageData.image;
        ...
        _onOpenDialog: function (e, imageData) {
            ...
            } else if(imageData['media_type'] === 'external-video') {
                this.showModal();
            }else{
                return;
            }
